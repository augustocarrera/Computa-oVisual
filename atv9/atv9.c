...
int main(int argc, char* argv[])
{
    ...
    // Criação e configuração do Vertex Buffer Object (VBO) para vértices.
    GLuint VBOs[2];
    glGenBuffers(2, VBOs);
    ...
    // Criação e configuração do Vertex Buffer Object (VBO) para cores.
    ...
    while (isRunning)
    {
        ...
        // Atualização contínua da rotação.
        rx += 0.5f;
        ry += 0.5f;
        rz += 0.5f;

        // Configuração da matriz do modelo (translação, rotação e escala).
        glm_mat4_identity(modelMatrix);
        ...
        // Multiplicação das matrizes de modelo, visão e projeção para obter a matriz MVP.
        mat4 MVPMatrix;
